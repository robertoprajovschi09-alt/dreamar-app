                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {cands.map((c) => (
                          <button key={c.id} onClick={() => { setChoose((p) => { const q = { ...p }; delete q[row.idx]; return q; }); void runOne(row, c.id); }}
                            className="rounded-full border border-border px-2.5 py-1 text-xs font-600 transition hover:border-[hsl(var(--strateg))] hover:bg-[hsl(var(--strateg))]/10">
                            {c.label}
                          </button>
                        ))}
                        <button onClick={() => { setChoose((p) => { const q = { ...p }; delete q[row.idx]; return q; }); setResults((p) => ({ ...p, [row.idx]: { ok: false, error: "Sărit: nu ai ales obiectul." } })); }}
                          className="rounded-full px-2.5 py-1 text-xs font-600 text-muted-foreground hover:bg-muted">
                          Sari peste
                        </button>
                      </div>
                    </div>
        ...
        {remaining > 0 ? (
          <>
            <button onClick={() => void apply()} disabled={applying || n === 0}
              className="inline-flex items-center gap-1.5 rounded-lg bg-[hsl(var(--strateg))] px-3 py-1.5 text-xs font-700 text-[hsl(var(--strateg-foreground))] transition hover:opacity-90 disabled:opacity-50">
