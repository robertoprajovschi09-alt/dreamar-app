...
import { clients as sampleClients, type Client, type Niche } from "@/data/sample";
...
function mapRow(r: any): Client {
  return {
    ...
    status: (r.status === "archived" ? "paused" : r.status) as Client["status"],
...
export function ClientsProvider({ children }: { children: ReactNode }) {
  ...
  const reload = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("clients")
      .select("id, name, niche, city, contact_person, contact_email, contact_phone, website, monthly_retainer, status, platforms, objectives, feedback")
    ...
    async (input: NewClientInput) => {
      if (!live || !supabase || !agencyId) {
        ...
        const c: Client = {
          ...
          retainer: input.retainer ?? 0, status: "onboarding", health: 0, risk: "low", platforms: input.platforms, trend: 0,
        };
        setClients((prev) => [c, ...prev]);
        setDetails((prev) => ({ ...prev, [id]: { objectives: input.objectives ?? [], feedback: "", email: "", phone: "", website: input.website ?? "" } }));
        return { id };
      }
      const { data, error } = await supabase
        .from("clients")
        .insert({
          ...
          monthly_retainer: input.retainer ?? null,
