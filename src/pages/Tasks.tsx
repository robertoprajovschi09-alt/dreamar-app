...
type Task = { id: string; title: string; client: string; clientId: string; type: string; priority: string; status: string; assignee: string; due: string; rawDeadline: string; overdue?: boolean };
...
export default function Tasks() {
  ...
  async function reload() {
    ...
    const { data, error } = await supabase
      .from("tasks")
      .select("id, title, task_type, priority, status, deadline, client_id, client:clients(name)")
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      return { id: r.id, title: r.title, client: r.client?.name ?? "", clientId: r.client_id ?? "", type: r.task_type, priority: r.priority, status: r.status, assignee: profile.name, due: r.deadline ? new Date(r.deadline).toLocaleDateString("ro-RO", { day: "numeric", month: "short" }) : "—", rawDeadline: r.deadline ? String(r.deadline).slice(0, 10) : "", overdue };
    }));
    setLoading(false);
  }
  ...
  async function createTask(input: { title: string; clientId: string; type: string; priority: string; deadline: string }) {
    if (!live || !supabase || !agencyId) {
      const t: Task = { id: "demo-" + Date.now(), title: input.title, client: clients.find((c) => c.id === input.clientId)?.name ?? "", clientId: input.clientId, type: input.type, priority: input.priority, status: "todo", assignee: profile.name, due: input.deadline ? new Date(input.deadline).toLocaleDateString("ro-RO", { day: "numeric", month: "short" }) : "—", rawDeadline: input.deadline };
      setTasks((prev) => [t, ...prev]);
      return {};
    }
    const { error } = await supabase.from("tasks").insert({ agency_id: agencyId, client_id: input.clientId || null, title: input.title, task_type: input.type, priority: input.priority, status: "todo", deadline: input.deadline || null });
    if (error) return { error: error.message };
    await reload();
    return {};
  }

  async function updateTask(id: string, input: { title: string; clientId: string; type: string; priority: string; deadline: string }) {
    if (!live || !supabase || !agencyId) {
      setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, title: input.title, clientId: input.clientId, client: clients.find((c) => c.id === input.clientId)?.name ?? "", type: input.type, priority: input.priority, due: input.deadline ? new Date(input.deadline).toLocaleDateString("ro-RO", { day: "numeric", month: "short" }) : "—", rawDeadline: input.deadline } : t)));
      return {};
    }
    const { error } = await supabase.from("tasks").update({ client_id: input.clientId || null, title: input.title, task_type: input.type, priority: input.priority, deadline: input.deadline || null }).eq("id", id);
    if (error) return { error: error.message };
    await reload();
    return {};
  }
  ...
  return (
    <>
      ...
      <TaskComposer open={composerOpen || !!editingTask} editing={editingTask} onClose={() => { setComposerOpen(false); setEditingTask(null); }} clients={clients}
...
function TaskComposer({ open, editing, onClose, clients, onSubmit }: {
  open: boolean; editing: Task | null; onClose: () => void; clients: { id: string; name: string }[];
  onSubmit: (input: { title: string; clientId: string; type: string; priority: string; deadline: string }) => Promise<{ error?: string }>;
}) {
  ...
  useEffect(() => {
    if (!open) return;
    if (editing) { setTitle(editing.title); setClientId(editing.clientId); setType(editing.type); setPriority(editing.priority); setDeadline(editing.rawDeadline); setBusy(false); }
    else { setTitle(""); setClientId(""); setType("other"); setPriority("medium"); setDeadline(""); setBusy(false); }
  }, [open, editing]);

  async function submit() {
    if (!title.trim() || busy) return;
    setBusy(true);
    const res = await onSubmit({ title: title.trim(), clientId, type, priority, deadline });
    setBusy(false);
