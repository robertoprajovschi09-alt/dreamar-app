...
const planTone: Record<string, any> = { Starter: "neutral", Growth: "primary", Unlimited: "info", "White Label Pro": "success" };
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
type Row = { id: string; name: string; owner: string; plan: string; clients: number; mrr: number; status: string };
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!active) return;
      if (error) console.error("[admin] load failed:", error.message);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      else if (data) setRows(data.map((a: any) => ({
        id: a.id, name: a.name, owner: a.owner_name || a.owner_email || "—",
        plan: TIER_SHORT[a.current_plan_tier] ?? a.current_plan_tier, clients: a.client_count ?? 0,
        mrr: Number(a.price_eur_monthly ?? 0), status: a.subscription_status ?? "trialing",
      })));
      setLoading(false);
    })();
    return () => { active = false; };
  }, [live, isSaasAdmin, agencyReady]);
  ...
  return (
    <>
      ...
      <Panel>
        ...
        <Table>
          <THead>
            <TH>Agenție</TH><TH>Proprietar</TH><TH>Plan</TH><TH>Clienți</TH><TH>MRR</TH><TH>Status</TH>
          </THead>
          <tbody>
            {rows.map((a) => (
              <TR key={a.id}>
                ...
                <TD><Badge tone={planTone[a.plan] ?? "neutral"}>{a.plan}</Badge></TD>
                <TD>{a.clients}</TD>
